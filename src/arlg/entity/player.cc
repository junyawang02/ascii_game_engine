...
Player::Player(int x, int y, size_t c) : Acceptor{x, y, make_unique<Still>(Bitmap{'p'}),
                                         ...
                                         iframe{true}, control{c} {}

void Player::doOnTick() {
    if (iframe)
        iframe = false;
    if (getActions()[control] == Action::W || getActions()[control] == Action::A || getActions()[control] == Action::S || getActions()[control] == Action::D) {
        iframe = true;
...
pair<Line, string> Player::doUpdateStatus() {
    string healthMessage = "Health: " + std::to_string((int)getHealth());
    return pair<Line, string>{Line::L1, healthMessage};
}

bool Player::invincible() { return iframe; }
